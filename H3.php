...
class H3
{
    ...
    public static function documentClass($className, $title = 'Usage')
    {
        ...
        $output = '## '.$title."\n\n";
        $output .= '> Public methods of *'.$className."*\n\n";

        foreach ($methods as $method) {
            $methodName = $method->getName();
            $startLine = $method->getStartLine();

            $doc = new kamermans\Reflection\DocBlock($reflection->getMethod($methodName));

            $output .= "### ".$methodName."()\n\n";
            $output .= "> ".$doc->getComment()."\n\n";
            ...
            foreach ($doc->getTags() as $tag => $value) {
                if (is_array($value)) {
